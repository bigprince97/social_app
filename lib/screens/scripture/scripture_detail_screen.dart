...
class _ScriptureDetailScreenState extends State<ScriptureDetailScreen> {
  ...
  void _openChapter(int index, {int? verse}) {
  ...
  Future<void> _openSearch() async {
    ...
    final result = await context.push<ScriptureSearchResult>(
      '/scripture/search/${scripture.id}',
      extra: {'scripture': scripture, 'chapters': _chapters},
    );
    if (!mounted || result == null) return;
    final index = _chapters.indexWhere((c) => c.id == result.chapter.id);
    if (index < 0) return;
    if (widget.initialBook != null) {
      context.pop(result.chapter.id);
      return;
    }
    _openChapter(index, verse: result.verseNumber);
  }
          ...
          if (_loading && _chapters.isEmpty)
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Center(
                  child: Text(AppLocalizations.of(context).noChapterContent),
                ),
              ),
            )
  ...
  SliverAppBar _buildAppBar(Scripture s) {
    final coverText = s.displayTitle.length > 2
        ? s.displayTitle.substring(0, 2)
        : s.displayTitle;
    ...
    return SliverAppBar(
      ...
      actions: [
        IconButton(
          ...
          onPressed: _chapters.isEmpty ? null : _openSearch,
                            ...
                            Row(
                              children: [
                                ...
                                Text(
                                  AppLocalizations.of(
                                    context,
                                  ).chaptersCountLabel(s.chaptersCount),
                                  style: TextStyle(
                                    color: Colors.white.withAlpha(200),
                                    fontSize: 12.5,
                                  ),
...
class _BibleDetailScreenState extends State<_BibleDetailScreen> {
  ...
  Widget _buildBookView(Map<String, List<ScriptureChapter>> bm) {
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          if (_otExpanded)
            _buildBookGrid(
              _otBooks.where((b) => bm.containsKey(b)).toList(),
              bm,
            ),
          _SectionHeader(
          ...
          if (_ntExpanded)
            _buildBookGrid(
              _ntBooks.where((b) => bm.containsKey(b)).toList(),
              bm,
            ),
  ...
  Widget _buildBookGrid(
    List<String> books,
    Map<String, List<ScriptureChapter>> bm,
  ) {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, i) {
          ...
          final abbr = isZh
              ? (_abbrev[name] ?? name.substring(0, 1))
              : localName;
          ...
          return GestureDetector(
            ...
            child: Container(
              decoration: BoxDecoration(
                color: _bg,
                border: Border.all(color: const Color(0xFFDDDDD0), width: 0.5),
              ),
  ...
  String _shortName(String name) {
    ...
    // 截短：去掉"书"、"记"、"福音" 等后缀
    final s = name.replaceAll('福音', '').replaceAll('书', '').replaceAll('记', '');
    return s.length <= 4 ? s : '${s.substring(0, 3)}…';
  }

  // ── 章 grid ───────────────────────────────────────────────────

  Widget _buildChapterView(String bookName, List<ScriptureChapter> chapters) {
    if (bookName.isEmpty || chapters.isEmpty) {
      return Center(
        child: Text(
          AppLocalizations.of(context).selectBookFirst,
  ...
  Widget _buildStatusBar(String bookName, List<ScriptureChapter> chapters) {
    ...
    if (_isBookView) {
      ...
      if (bookName.isNotEmpty && chapters.isNotEmpty) {
        currentNum = _localNum(chapters.first);
      }
      display = bookName.isNotEmpty
          ? AppLocalizations.of(
              context,
            ).bookChapterDisplay(bookName, currentNum)
          : widget.scripture.displayTitle;
    }
    return Container(
      height: 44,
        ...
        child: Row(
          children: [
            ...
            Text(
              ...
              style: const TextStyle(fontSize: 12, color: Color(0xFFAA88CC)),
            ),
          ],
        ),
      ),
...
class _DefaultContents extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final lang = LocaleController.instance.bibleLang == 'zh_Hant'
        ? 'zh_Hant'
        : 'zh';
    return SliverList(
      delegate: SliverChildBuilderDelegate((context, i) {
            ...
            child: Row(
              children: [
                Container(
                  ...
                  height: 28,
                  decoration: BoxDecoration(
                    color: ch.isBookmarked ? color : color.withAlpha(20),
                    shape: BoxShape.circle,
                  ),
                  child: Center(
